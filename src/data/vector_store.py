...
class VectorStoreManager:
    ...
    def __init__(self):
        ...
        logger.info(f"Initialisation du modèle embeddings : {app_config.models.embeddings_model} sur le device '{device}'...")
    ...
    def _load_index_and_chunks(self):
        ...
        if os.path.exists(app_config.paths.faiss_index_file) and os.path.exists(app_config.paths.document_chunck_file):
            try:
                logger.info(f"Chargement de l'index Faiss depuis {app_config.paths.faiss_index_file}...")
                self.index = faiss.read_index(app_config.paths.faiss_index_file)

                logger.info(f"Chargement des chunks depuis {app_config.paths.document_chunck_file}...")
                with open(app_config.paths.document_chunck_file, "rb") as file:
                    self.document_chunks = pickle.load(file)

                logger.info(f"Index ({self.index.ntotal} vecteurs) et {len(self.document_chunks)} chunks chargés.")
            except Exception as e:  # noqa: BLE001
                logger.error(f"Erreur lors du chargement de l'index / chunks : {e}")
                self.index = None
    ...
    def _split_documents_to_chunks(self, documents : list[Document]) -> list[dict[str, Any]]:
        ...
        logger.info(f"Découpage de {len(documents)} documents en chunks (taille={app_config.indexer.chunk_size}, chevauchement={app_config.indexer.chunk_overlap})...")
        ...
        logger.info(f"Total de {len(all_chunks)} chunks créés.")
        return all_chunks
    ...
    def _generate_embeddings(self, chunks : list[dict[str, Any]]) -> np.ndarray | None:
        ...
        logger.info(f"Génération des embeddings pour {len(chunks)} chunks (modèle: {app_config.models.embeddings_model})...")
        all_embeddings = []
        ...
        for i in range(0, len(chunks), app_config.indexer.embedding_batch_size):
            ...
            logger.info(f"  Traitement du lot {batch_num}/{total_batches} ({len(texts_to_embed)} chunks)")

            try:
                batch_embeddings = self.embedder.embed_documents(texts_to_embed)
                all_embeddings.extend(batch_embeddings)
            except Exception as e: # noqa: BLE001
                logger.error(f"Erreur lors de la génération d'embeddings (lot {batch_num}): {e}")
                ...
                logger.warning(f"Ajout de {num_failed} vecteurs nuls de dimension {dim} pour le lot échoué.")
        ...
        logger.info(f"Embeddings générés avec succès. Shape: {embeddings_array.shape}")
    ...
    def _save_index_and_chunks(self):
        ...
        try:
            ...
            logger.info(f"Sauvegarde de l'index Faiss dans {app_config.paths.faiss_index_file}...")
            faiss.write_index(self.index, app_config.paths.faiss_index_file)

            logger.info(f"Sauvegarde des chunks dans {app_config.paths.document_chunck_file}...")
            ...
            logger.error(f"Erreur lors de la sauvegarde de l'index/chunks : {e}")
    ...
    def build_index(self, documents : list[Document]):
        ...
        if embeddings is None or embeddings.shape[0] != len(self.document_chunks):
            logger.error("Problème de génération d'embeddings. Le nombre d'embeddings ne correspond pas au nombre de chunks.")
        ...
        logger.info(f"Création de l'index Faiss optimisé pour la similarité cosinus avec dimension {dimension}...")
